...
API.prototype.openWallet = function(cb) {
  $.checkState(this.credentials);
  var self = this;
  console.log('checking if wallet complete');
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  var r = ""
  if (url.indexOf('v4') >= 0) {
    r = self.request[method]("http://localhost:8080" + url);
  } else {
    r = self.request[method](self.baseUrl + url);
  }
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  self._doPostRequest('/v4/createWallet', args, function(err, res) {
    if (err) return cb(err);
...
API.prototype.createAddress = function(opts, cb) {
  ...
  opts = {
    'walletId': this.credentials.walletId,
    'publicKeyRing': this.credentials.publicKeyRing
  };

  //self._doPostRequest('/v3/addresses/', opts, function(err, address) {
  self._doPostRequest('/v4/addresses/', opts, function(err, address) {
    if (err) return cb(err);
...
API.prototype.broadcastTxProposal = function(txp, cb) {
  ...
  var self = this;
  txp.coin = this.credentials.coin;
...
API.prototype.signMessage = function(text, privKey, coin) {
  return Utils.signMessage(text, privKey, coin);
};

API.prototype.verifyMessage = function(text, signature, pubKey, coin) {
  return Utils.verifyMessage(text, signature, pubKey, coin);
}
