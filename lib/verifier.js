...
Verifier.checkTxProposalSignature = function(credentials, txp) {
  ...
  // For backwards compatibility with the Tarascash wallet
  if (!creatorKeys) {
    creatorKeys = _.find(credentials.publicKeyRing, function(item) {
      if (Utils.xPubToCopayerId('btc', item.xPubKey) === txp.creatorId) return true;
    });
  }
