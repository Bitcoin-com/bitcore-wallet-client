...
API.prototype._import = function(cb) {
  ...
  self.migrateWallet(function(err, isMultiSig) {
    if (isMultiSig) {
      self.baseUrl = self.credentials.coin == 'btc' ? 'https://bws.bitcoin.com/bws/api' : 'https://bwscash.bitcoin.com/bws/api';
    }
 ...
 * migrate wallet
 *
 * @return {Callback} cb - Returns if this wallet is a multisig wallet
 */
API.prototype.migrateWallet = function(cb) {
  $.checkState(this.credentials);
  var self = this;
  var coin = self.credentials.coin;
  var url = '/v1/migrate/migration/' + coin;
  this._doGetRequest(url, function(err, response) {
    ...
    return cb(null, response.multiSig);
  });
};
