...
PayPro.get = function(opts, cb) {
  ...
  http(opts, function(err, dataBuffer) {
    ...
    var mapOutputs = function(output){
      ...
      return {addr: addr.toString(), amount: amount}
    };

    var amountReduce = function (a, b) {
      return b['amount'] == null ? a : a + b['amount'];
    };

    var outs = outputs.map(mapOutputs);

    var totalAmount = outs.reduce(amountReduce, 0);
